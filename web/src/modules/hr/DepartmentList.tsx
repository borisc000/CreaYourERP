...
export function DepartmentList() {
  ...
  const { data: departments, isLoading } = useFirestoreCollection<Department>("departments", [orderBy("name")]);
  const { create, remove } = useFirestoreDoc<Department>("departments");
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await create({ name: newName, code: newCode || undefined, isActive: true } as Omit<Department, "id" | "companyId" | "createdAt">);
    setNewName("");
    setNewCode("");
  };
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-white mb-6">Departamentos</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 text-gray-500">
          <BuildingOfficeIcon className="w-10 h-10 mx-auto mb-2 text-gray-600" />
          ...
          {departments.map((dept) => (
            <div key={dept.id} className="flex items-center justify-between p-4 group">
              <div>
                ...
                {dept.code && <p className="text-gray-500 text-xs">{dept.code}</p>}
              </div>
              <button
