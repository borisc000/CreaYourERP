...
export const rejectQuote = onCall(
  ...
  async (request) => {
    ...
    if (!["draft", "sent"].includes(currentStatus)) {
      throw new HttpsError("failed-precondition", `No se puede rechazar una cotización en estado ${currentStatus}`);
    }
    ...
    await db.runTransaction(async (t) => {
      ...
      t.set(companyRef(companyId).collection("activityLogs").doc(), {
        ...
        type: "quote.rejected",
        quoteId,
        message: `Cotización ${quote.quoteNumber} rechazada`,
    ...
    return { id: quoteId, status: "rejected" };
  }
);
