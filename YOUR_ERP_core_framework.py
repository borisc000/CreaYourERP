...
class Request:
    ...
    files: Dict[str, Any] = field(default_factory=dict)  # Archivos subidos
...
class CoreFramework:
    ...
    async def dispatch_request(self, request: Request) -> Response:
        ...
        if not request.is_authenticated():
            request.user_id = 0  # Usuario anónimo
        ...
        try:
            import re
            # Buscar ruta en módulos
            for module in self.module_registry.get_all_loaded().values():
                routes = module.get_routes()
                
                for path, route_config in routes.items():
                    # Coincidencia expandida a Regex para parsear {id} y dinamicos
                    path_regex = "^" + re.sub(r'\{([^}/]+)\}', r'(?P<\1>[^/]+)', path) + "$"
                    match = re.match(path_regex, request.path)
                    
                    if match and request.method in route_config['methods']:
                        
                        # Inyectar variables capturadas ({id}, etc) directo en params
