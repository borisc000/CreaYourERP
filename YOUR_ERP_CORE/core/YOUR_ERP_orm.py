...
class BaseModel(ABC, metaclass=BaseModelMeta):
    ...
    def save(self) -> bool:
        ...
        try:
            ...
            if self._id is None:
                # INSERT - asignar ID autoincremental
                self.before_insert()

                if table not in BaseModel._store:
                    BaseModel._store[table] = {}
                    BaseModel._id_counters[table] = 0
        ...
        if domain:
            for field_name, operator, value in domain:
                ...
                for record in results:
                    rec_val = record._data.get(field_name, getattr(record, field_name, None))
                    if operator == '=' and rec_val == value:
                        filtered.append(record)
                    elif operator == '!=' and rec_val != value:
                        filtered.append(record)
                    elif operator == '<' and rec_val is not None and rec_val < value:
                        filtered.append(record)
                    elif operator == '>' and rec_val is not None and rec_val > value:
                        filtered.append(record)
                    elif operator == '<=' and rec_val is not None and rec_val <= value:
                        ...
                        filtered.append(record)
                    elif operator == 'in' and rec_val in value:
                        filtered.append(record)
                    elif operator == 'not in' and rec_val not in value:
                        filtered.append(record)
                    elif operator == 'like' and rec_val and value.replace('%', '') in str(rec_val):
                        filtered.append(record)
                    elif operator == 'ilike' and rec_val and value.replace('%', '').lower() in str(rec_val).lower():
                        filtered.append(record)
                results = filtered
