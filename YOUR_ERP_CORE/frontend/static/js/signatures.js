...
function filteredRequests(items) {
    ...
    return items.filter((item) => {
        const itemOrigin = sigOriginValue(item);
        const haystack = [
            item.name,
            item.document_name,
        ...
        const matchesOrigin = !origin || itemOrigin === origin;
            ...
            <div style="display:flex;justify-content:space-between;gap:0.75rem;align-items:flex-start;">
                ...
                <div style="display:flex;flex-direction:column;gap:0.45rem;align-items:flex-end;">
                    ...
                    <div class="sig-row-actions">
                        ...
                        <button class="btn btn-ghost btn-sm" onclick="openSignatureDetail(${item.id})">Bitácora</button>
                    </div>
                </div>
            </div>
        </div>
...
function renderCreatedRequests() {
    ...
    const created = filteredRequests(signatureState.created);
    if (!created.length) {
        body.innerHTML = '<tr><td colspan="5" class="empty">No hay solicitudes creadas en este filtro.</td></tr>';
        return;
    }

    body.innerHTML = created.map((item) => `
        <tr>
            <td>
                <div style="display:flex;flex-direction:column;gap:0.2rem;">
                    <strong style="color:#f8fafc;">${sigEscape(item.name || '-')}</strong>
                    <span class="text-sm text-muted">${sigEscape(item.document_name || '-')}</span>
                </div>
            </td>
            ...
            <td style="white-space:nowrap;">
                ${item.status === 'draft' ? `<button class="btn btn-ghost btn-sm" onclick="sendRequestNow(${item.id})">Enviar</button>` : ''}
...
function renderCompanyPipeline() {
    ...
    body.innerHTML = company.map((item) => `
        <tr>
            ...
            <td>
                <div style="display:flex;flex-direction:column;gap:0.2rem;">
                    <span class="text-sm text-muted">Solicitante #${sigEscape(item.request_from || '-')}</span>
                    <span>${sigEscape(item.request_to_email || '-')}</span>
                </div>
            </td>
            <td>${sigStatusPill(item.status)}</td>
            <td>
                <div style="display:flex;flex-direction:column;gap:0.2rem;">
                    <span>${sigEscape(sigOriginLabel(item))}</span>
                    <span class="text-sm text-muted">${sigEscape(item.source_model || item.generated_document_id ? `#${item.generated_document_id || item.source_record_id || '-'}` : '-')}</span>
                </div>
            </td>
...
async function createRequest(event) {
    ...
    const payload = {
        ...
        document_name: document.getElementById('sig-file').value || pdfFile?.name || 'document.pdf',
        signature_positions: [{ page: 0, x: 100, y: 500, width: 200, height: 80 }],
        auto_send: true,
    };
    if (pdfFile) {
        payload.document_data = await readFileAsBase64(pdfFile);
    }
...
function mountSignatureWorkspace() {
    ...
    signatureState.workspace = PdfSignatureWorkspace.create(host, {
        title: detail.status === 'signed' ? 'PDF firmado y sellado' : 'Ubicacion visual de firma sobre PDF',
        ...
        onChange(nextPositions) {
            if (signatureState.active) {
                signatureState.active.signature_positions = nextPositions;
            }
        },
    });
...
async function saveSignatureLayout() {
    const detail = signatureState.active;
    if (!detail?.id || !signatureState.workspace) return;
    const response = await API.post(`/signature/requests/${detail.id}/layout`, {
    ...
    signatureState.active.signature_positions = response.data?.signature_positions || [];
    signatureState.active.pdf_layout = response.data?.pdf_layout || signatureState.active.pdf_layout || [];
...
function renderSignatureDetail() {
    ...
    body.innerHTML = `
        ...
        <div style="display:flex;gap:0.75rem;flex-wrap:wrap;margin:1rem 0;">
            ...
            ${detail.public_url ? `<button class="btn btn-ghost" onclick="openPublicSignature('${sigEscape(detail.public_url)}')">Abrir enlace público</button>` : ''}
            ${detail.status === 'draft' ? `<button class="btn btn-primary" onclick="sendRequestNow(${detail.id})">Enviar ahora</button>` : ''}
        ...
        <div class="sig-grid">
            <div class="card" style="margin:0;">
                ...
                <dl class="doc-info">
                    ...
                    <dt>Descripción</dt><dd>${sigEscape(detail.description || '-')}</dd>
                </dl>
            </div>
                ...
                <div class="table-wrap">
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            `).join('') : '<tr><td colspan="3" class="empty">Sin eventos registrados aún.</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
