/* ============================================================
   CRM_CUSTOMERS.JS  — Customers Table Page
   ============================================================ */
...
async function loadCustomers() {
    ...
    if (!res || !res.success) {
        document.getElementById('cust-tbody').innerHTML =
            '<tr><td colspan="7" style="text-align:center;padding:2rem;color:#ef4444;">Error cargando clientes</td></tr>';
        return;
    }
...
function renderTable(list) {
    ...
    if (!list.length) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:2.5rem;color:#64748b;">Sin resultados</td></tr>';
        return;
    }
...
function goToPipeline(customerId) {
...
function openEditModal(id) {
    const c = CUSTS.all.find(x => x.id === id);
    if (!c) return;
...
async function deleteCustomer() {
    const id = document.getElementById('cust-id').value;
    if (!id) return;
    const c = CUSTS.all.find(x => x.id === parseInt(id));
    if (!confirm(`¿Eliminar cliente "${c?.name || ''}"?\nSus oportunidades quedarán sin cliente asignado.`)) return;
...
async function saveMandante() {
    ...
    if (res && res.success) {
        ...
        closeMandanteModal();
        await loadCustomers(); // Refresh table to increment the count
    } else {
