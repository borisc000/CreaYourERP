def base_layout(title, page_id, content, scripts=None, no_sidebar=False):
    import sys
    print(f"DEBUG: base_layout called with scripts={scripts}", file=sys.stderr, flush=True)
    sidebar = ""
    ...
    if scripts:
        for s in scripts:
            scripts_html += f'<script src="/static/js/{s}?v=9.9.9-DEBUG"></script>\n'
...
<link rel="stylesheet" href="/static/css/theme.css?v=UNIQUE-MARKER-12345">
</head>
<body>
